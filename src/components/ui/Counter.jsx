...
function Number({ mv, number, height }) {
  ...
  // Calculate fading opacity based on proximity to center focus
  let opacity = useTransform(y, yVal => {
    const dist = Math.abs(yVal) / height;
    return Math.max(0, 1 - dist * 1.6);
  });

  return (
    <motion.span className="counter-number" style={{ y, opacity }}>
      {number}
    </motion.span>
  );
}
